...
from riskfolio_beta.core import AttributionEngine, ScenarioEngine, Scenario
...
by_product = AttributionEngine.attribution_table(df, dims=("product",))
print(by_product)
...
scenario = Scenario(
    weights = {
        "US_Treasury": 0.8,   # 美国国债风险敞口降低
        "Corp_Bond": 1.2,     # 公司债暴露增加
        "MBS": 1.0            # MBS 不变
    },
    level = "product",        # 根据 product 这一列匹配权重
    col = "notional"          # 调整 notional 这一列
)


scenario_engine = ScenarioEngine()

df_alt = scenario_engine.apply(df, scenario)
by_product_alt = AttributionEngine.attribution_table(df_alt, dims=("product",))
